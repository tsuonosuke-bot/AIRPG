...
public static class HelpScreen
{
    ...
    static async Task ShowBattleAsync()
    {
        Ui.BeginScreen();
        Ui.Header("戦闘");
        Ui.WriteLine("  ・命中判定        : 1d20に命中補正を足し、相手の回避値(DV)を上回れば命中する。");
        Ui.WriteLine("                      DVは6を基準に敏捷と装備で増減する。重い鎧はDVを下げる。");
        Ui.WriteLine("  ・貫通判定        : 命中した後、攻撃側の貫通値(PV)と防御側の装甲値(AV)を突き合わせる。");
        Ui.WriteLine("                      1d10-2にPVを足してAVと比べる試行を3回で1セット。");
        Ui.WriteLine("                      1回でも上回れば1貫通。3回とも上回るとPVを2下げて次のセットへ進み、");
        Ui.WriteLine("                      貫通を積み増していく。1回も上回らなければ装甲に弾かれる。");
        Ui.WriteLine("  ・ダメージ        : 貫通した回数ぶんだけ武器のダメージダイスを振って合計する。");
        Ui.WriteLine("                      能力値は直接ダメージに乗らない。効くのは「何回貫通したか」だけ。");
        Ui.WriteLine("                      貫通できなければダメージは0で、最低保証はない。");
        Ui.WriteLine("  ・PVの作り方      : 武器そのもののPVに、筋力(魔法は知力)のmodifierを足す。");
        Ui.WriteLine("                      ただし武器ごとに乗せられる上限があり、短剣や投石は腕力を");
        Ui.WriteLine("                      乗せきれず頭打ちになる。斧や大剣は上限がなく青天井に伸びる。");
        Ui.WriteLine("                      力自慢には重い得物を、器用な者には軽い得物を持たせるとよい。");
        Ui.WriteLine("  ・物理/魔法       : どちらで殴るかは装備した武器で決まる。杖なら知力をPVに乗せて");
        Ui.WriteLine("                      相手の魔法装甲(mAV)と、それ以外は筋力を乗せて装甲(AV)と突き合わせる。");
        Ui.WriteLine("  ・会心            : 1d20の素の出目が20なら、DVに関わらず命中する。");
        Ui.WriteLine("                      さらにPVが+1され、1回も抜けなくても最低1貫通は通る。");
