...
public static class HelpScreen
{
    ...
    static async Task ShowMasteryAsync(GameMasterData db)
    {
        ...
        Ui.WriteLine("     報酬・道中イベント・休息・敵ドロップ・行軍速度を動かすスキルは、連れて行った全員ぶんが合算される。");
        Ui.WriteLine("     隊列も生死も関係なく、その遠征に誰を出したかだけで決まる。");
        Ui.WriteLine("     行軍（幌馬車）は1ターンに踏み込むエリア数を増やす。踏むエリアの総数は変わらないので、");
        Ui.WriteLine("     道中の出来事を飛ばすことなく、同じ依頼をより少ないターンで終えて帰ってこられる。");
        Ui.WriteLine("     合算後の効果は、クエストの編成確認画面「遠征補正（隊全体）」でまとめて確認できる。");
        Ui.WriteLine();
