...
public static class HelpScreen
{
    ...
    static async Task ShowBattleAsync()
    {
        ...
        Ui.WriteLine("  ・ダメージ        : 武器のダメージダイスが基礎値。これを攻撃力で増幅し、");
        Ui.WriteLine("                      相手の防御力を引いたものが最終ダメージになる。");
        Ui.WriteLine("                      つまり火力の土台は装備であり、能力値はその倍率として効く。");
        Ui.WriteLine("  ・増幅上限        : 武器ごとに「能力をどこまで乗せられるか」の上限がある。");
        Ui.WriteLine("                      短剣や投石は腕力を乗せきれず頭打ちになり、斧や大剣は青天井に伸びる。");
        Ui.WriteLine("                      力自慢には重い得物を、器用な者には軽い得物を持たせるとよい。");
        Ui.WriteLine("  ・物理/魔法       : どちらで殴るかは装備した武器で決まる。杖なら魔法攻撃と魔法防御、");
        Ui.WriteLine("                      それ以外は物理攻撃と物理防御で計算する。");
        Ui.WriteLine("  ・決定的成功      : 命中判定で特に良い出目を出すと、武器ダイスをもう1セット振って加算する。");
