...
延長という逃げ道があるので「絶対に届く」水準までは要りませんが、**毎回延長になるようでは
予定が予定になりません**。「戦闘で負けた」ではなく「抽選が渋かった」で行程が延びるのは
プレイヤーに手の打ちようがないので、通常は予定内で収まる余裕を数値の側に持たせます。

マスタの数値は次の2つを満たすように決めます（`MasterBandChecker` が検査します）。

```
採取できるフェーズ数 = totalPhases −（ボス付きなら 1）
採取の期待量         = 採取できるフェーズ数 × gatherChance × (gatherMin + gatherMax) ÷ 2

1. gatherChance ≥ 0.65
2. 採取の期待量 ≥ gatherTargetCount × 1.5
```

2つ目だけでは足りません。`gatherChance` が低いと当たる回数そのものが少なく、
期待量に余裕を持たせても1本ごとのブレが大きいままだからです。両方を満たすと、
**予定フェーズ内での達成が95%前後**に収まり、残りの5%ほどが延長判断に回ります。

倍率と延長判断の起きやすさの対応は次のとおりです。緊張の量を変えたいときはここを動かします。

| `GatherSurplusFactor` | 予定内で達成 | 延長判断が出る頻度 |
| --- | --- | --- |
| 2.0 | 約98% | 50本に1本 |
| **1.5**（現行） | 約95% | **20本に1本** |
| 1.35 | 約92% | 12本に1本 |
...
| クエスト | ランク | `totalPhases` | `gatherChance` | 1回の採取量 | 目標数 | 期待量 | 余裕 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `quest_herb` | F | 10 | 0.8 | 1〜4 | 13 | 20.0 | 1.54 |
| `quest_moonflower` | F | 10 | 0.7 | 1〜4 | 11 | 17.5 | 1.59 |
| `quest_investigate_woods` | F | 10 | 0.75 | 2〜4 | 15 | 22.5 | 1.50 |
| `quest_wild_berries` | F | 10 | 0.75 | 1〜4 | 12 | 18.8 | 1.56 |
| `quest_clover_search` | F | 10 | 0.7 | 1〜2 | 7 | 10.5 | 1.50 |
| `quest_iron_ore` | E | 14 | 0.7 | 2〜5 | 22 | 34.3 | 1.56 |
| `quest_cave_mushroom` | E | 11 | 0.7 | 2〜4 | 15 | 23.1 | 1.54 |
| `quest_grave_moss` | D | 15 | 0.7 | 2〜5 | 24 | 36.8 | 1.53 |
| `quest_old_city_relic` | E | 14 | 0.7 | 1〜4 | 16 | 24.5 | 1.53 |

**素材の希少さは `gatherChance` ではなく採取量の幅（`gatherMin`／`gatherMax`）で表してください。**
`quest_clover_search` は1回に1〜2枚しか見つからない代わりに目標数が7枚で、
「探し歩けば見つかるが、一度にはまとまらない」希少さになっています。
`gatherChance` を下げて希少さを出すと、採取判定の回数そのものが減り、
希少さではなく理不尽さになります。
...
前掲「採取クエスト」を参照してください（`gatherTargetCount` は採取の期待量の
1/1.5 以下に置き、予定フェーズ内で95%前後が収まるようにします）。
