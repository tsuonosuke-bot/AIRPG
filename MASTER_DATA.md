...
`classSkills` の `requiredClearCount` は、解禁に必要な「**クラス習熟度**」です。
内部では習熟ポイントを100ptためるごとに習熟度1として扱い、0 なら職業に就いた瞬間に手に入ります。

習熟ポイントが入るのは、次をすべて満たしたときだけです（`AdventurerData.OnClearQuest`）。
...
1回の獲得量は `100 + clamp(INT, 0, 30)` pt です。通常の基礎値は100ptで、INT 1につき1pt増え、
上限は130pt（基礎値の+30%）です。INTが低くても基礎値を下回ることはありません。
たとえば INT 4 は104pt、INT 15 は115ptを獲得します。
...
Lv5 は習熟度88（8,800pt）と遠いので、**終盤の目標**として置いてある想定です。
