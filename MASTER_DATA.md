...
**採取クエストの達成条件は素材が揃ったかどうかだけ**です。予定フェーズを踏破しても手ぶらなら
...
- 余分に1ターンぶんの維持費を払う
- 踏み足すフェーズでも戦闘・罠・士気の消耗は起きる。粘るほど全滅と士気崩壊が近づく
...
達成率は `RankBandTable.GatherSuccessPercent` が**厳密に計算**します（フェーズごとの畳み込み）。
「期待量が目標の何倍か」という近似では足りません。同じ期待量でも「毎フェーズ1〜2個」と
「たまに大量」ではブレの幅がまるで違い、達成率が10ポイント以上ずれるためです。
`MasterBandChecker` がこの計算値を上の帯と突き合わせます。

```
採取できるフェーズ数 = totalPhases −（ボス付きなら 1）
```
...
C以上は舞台を新設せず、既存7ダンジョンの深いフェーズを `encounterTable` の
...
前掲「採取クエスト」を参照してください（予定フェーズ内に目標へ届く確率を
F の76〜84%からSの60〜68%まで、ランクが上がるほど下げます）。

### 経験値

戦闘の経験値は**参加者全員に満額**入り、クエスト完了報酬の `rewardExp` だけが
**人数で頭割り**されます（`QuestProgressor.cs` / `QuestRewardService.cs`）。
下の逆算は「6人パーティ・遭遇率25%・1戦闘あたり敵3体」を前提にしています。

| ランク | F | E | D | C | B | A | S |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 帯を抜けるのに要るexp（1人） | 1,000 | 2,250 | 4,350 | 8,600 | 11,800 | 15,000 | 20,700 |
| 1本で1人が得るexp | 200 | 375 | 620 | 1,075 | 1,310 | 1,500 | 2,070 |
| 敵1体の `exp` | 12〜20 | 16〜26 | 20〜32 | 28〜42 | 30〜46 | 32〜50 | 38〜60 |
| クエストの `rewardExp` | 300〜600 | 600〜1,100 | 1,100〜1,800 | 2,000〜3,000 | 2,600〜3,700 | 3,000〜4,300 | 4,200〜6,000 |

次レベルに要るexpは `100 + (Lv−1) × 50` です（システム側の固定値）。

| 到達Lv | 5 | 10 | 16 | 24 | 32 | 40 | 50 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 累計exp | 700 | 2,700 | 6,750 | 14,950 | 26,350 | 40,950 | 63,700 |

### ゴールド

維持費は `ギルド基本10 + Σ(冒険者Lv × 3 + (冒険者ランク − 1) × 15) + 施設の維持費` です（`GuildManager.cs`）。
...
| ランク | F | E | D | C | B | A | S |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 維持費の目安（G/ターン） | 100 | 265 | 445 | 685 | 940 | 1,195 | 1,480 |
