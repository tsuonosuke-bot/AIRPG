...
**目標数に届かないまま最終フェーズを迎えると撤退**になり、報酬はゼロです。
途中まで集めたぶんの部分点はありません（`QuestProgressor.AdvanceOnePhase`）。
つまり採取クエストは全部か無かの判定なので、**負ける理由は「戦闘や士気で保たなかった」だけ**に
しておきたい。「抽選が渋くて足りなかった」はプレイヤーに手の打ちようがなく、
やり直すしかない負け方になります。

そのために、マスタの数値は次の2つを満たすように決めます（`MasterBandChecker` が検査します）。
...
1. gatherChance ≥ 0.65
2. 採取の期待量 ≥ gatherTargetCount × 2
```

2つ目だけでは足りません。`gatherChance` が低いと当たる回数そのものが少なく、
期待量に余裕を持たせても1本ごとのブレが大きいままだからです
（`gatherChance` 0.4 だと、期待量が目標の2倍あっても未達が13%まで出ます）。
両方を満たすと、採取抽選だけを理由にした失敗は**5%未満**に収まります。

期待量が目標の2倍あるので、目標数にはおおむね**行程の半ば**で届きます。
残りのフェーズは「すぐ帰る」か「宝箱を狙って奥へ進む」かの選択に使われ、
そこから先の危険はプレイヤーが選んで背負ったものになります。

現在の採取クエストは次のとおりです。

| クエスト | ランク | `totalPhases` | `gatherChance` | 1回の採取量 | 目標数 | 期待量 | 余裕 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `quest_herb` | F | 10 | 0.8 | 1〜4 | 10 | 20.0 | 2.00 |
| `quest_moonflower` | F | 10 | 0.7 | 1〜4 | 8 | 17.5 | 2.19 |
| `quest_investigate_woods` | F | 10 | 0.75 | 2〜4 | 10 | 22.5 | 2.25 |
| `quest_wild_berries` | F | 10 | 0.75 | 1〜4 | 9 | 18.8 | 2.08 |
| `quest_clover_search` | F | 10 | 0.7 | 1〜2 | 5 | 10.5 | 2.10 |
| `quest_iron_ore` | E | 14 | 0.7 | 2〜5 | 15 | 34.3 | 2.29 |
| `quest_cave_mushroom` | E | 11 | 0.7 | 2〜4 | 10 | 23.1 | 2.31 |
| `quest_grave_moss` | D | 15 | 0.7 | 2〜5 | 18 | 36.8 | 2.04 |
| `quest_old_city_relic` | E | 14 | 0.7 | 1〜4 | 12 | 24.5 | 2.04 |

**素材の希少さは `gatherChance` ではなく採取量の幅（`gatherMin`／`gatherMax`）で表してください。**
`quest_clover_search` は1回に1〜2枚しか見つからない代わりに目標数が5枚で、
...
前掲「採取クエスト」を参照してください（目標未達は報酬ゼロの撤退になるため、
`gatherTargetCount` は期待量の半分以下に置きます）。
