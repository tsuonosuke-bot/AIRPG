...
編成画面では、冒険者を選ぶと武器の間合いに合った位置（近接なら前衛、弓・魔法・回復なら後衛）へ
自動で配置されます。位置の入れ替えや除外は「配置を変える / 外す」から行えます。
できあがった編成は名前を付けて6件まで保存でき、次の受注では「保存した編成を呼び出す」で
遠征方針ごと呼び戻せます。保存し忘れても、直前に出発したパーティは「前回の編成」として
同じ一覧に残ります。呼び出した時点で死亡・解雇・遠征中の隊員は自動的に外れ、
誰が抜けたのかが編成画面に表示されます。
